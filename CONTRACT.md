...
| nasdaq_deviation | timeseries | yahoo    | N      | 나스닥(^IXIC) 200일선 이격도 = (지수÷200D MA−1)×100 (%). 김성환 ① 대전제 "이격도=꿈" 매핑(charts-mapping N2). section=밸류에이션 |
...
| kr_fear_greed     | timeseries | naver+yahoo | N | **KOSPI 공포·탐욕 지수 (0~100, 수급 가중 자체 산식)**. 설계 원칙: 한국 시장은 외국인/기관 수급이 최중요인데 해외 F&G는 과소 반영 → 수급 합산 55% 가중. 요인 5개 = ①외국인 20일 누적 순매수 **35%** ②기관 20일 누적 순매수 **20%** ③KOSPI 50일선 이격도 **20%** ④20일 실현변동성(역방향) **15%** ⑤52주 신고가 대비 위치 **10%**. 각 요인을 최근 1년(수급 데이터 가용 창) 표본 내 백분위(rank pct×100)로 정규화 후 가중평균. 구간: <25 극공포 / 25~45 공포 / 45~55 중립 / 55~75 탐욕 / >75 극탐욕. markLines 25·75. **추가 fetch 없음** — kr_foreign_flow(Naver 표, 28페이지≈280영업일로 확대 수집·모듈 캐시)와 kr_rotation_check(^KS11 캐시) 데이터 재사용. 변동성은 VKOSPI 무키 부재로 실현변동성 프록시. **데일리 시드 제외** (스냅샷 카드로 충분, 차트는 전체 탭) |
...
- Naver 페이지네이션(페이지당 10영업일)은 28페이지 ≈ 280영업일을 **1회 수집 후 모듈 캐시** — kr_foreign_flow는 뒤 160영업일만 표시(기존 범위 유지), kr_fear_greed는 전체 창을 백분위 분포로 사용. 영업일만 내려오므로 휴일 방어 별도 불필요.
