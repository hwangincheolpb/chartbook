    ...
    if eps_pairs:
        ...
        if eps_window:
            ...
            return {
                ...
                "updated": _now_kst(),
                "note": (
                    "지수 레벨이 아니라 무엇이 밀어올리는가를 본다. EPS 밴드 안에서 오르면 실적장, "
                    "밴드 상단을 이탈해 멀티플만 확장하면 드림장(버블 초입) — 그건 탈출 신호가 아니라 "
                    "국면 전환 신호다. "
...
def fetch_vix(lookback_years: int = 6) -> dict[str, Any]:
    ...
    return {
        ...
        "updated": _now_kst(),
        "note": (
            "불안감이 살아있는 동안엔 버블이 아니다. 공포가 다 사라지고 '무조건 사'가 들릴 때가 "
            "진짜 버블 — 이 게이지가 조용해질수록 출구를 그려라. "
            "[출처] 이선엽 프레임워크 §7-6 (공포 역설, C2 버블 판별 보조). "
            "[한계] VIX 단독 표시 — 원 스펙의 KOSPI 2축 오버레이는 미적용."
        ),
        "series": [
...
def fetch_yield_spread(lookback_years: int = 6) -> dict[str, Any]:
    ...
    return {
        ...
        "unit": "%",
        "updated": _now_kst(),
        "note": "0선 아래로 내려가면 장단기 금리 역전 → 경기침체 선행 신호.",
        "series": [
